...
static Adafruit_NeoPixel internalLed(NUM_PIXELS, NEOPIXEL_PIN, NEO_GRB + NEO_KHZ800);
static bool internalLedReady = false;
static uint8_t ledLastR = 255, ledLastG = 255, ledLastB = 255;
...
static void drawNotificationInternal(const char* title, const char* message, bool showSave) {
    ...
    tft.fillRect(messageBoxX, messageBoxY, messageBoxWidth, messageBoxHeight, 0x9FF3);
    tft.setTextColor(BLACK, 0x9FF3);
    ...
    if (showSave) {
      ...
      tft.drawLine(okButtonX, okButtonY, okButtonX + okButtonWidth, okButtonY, 0x9FF3);
      tft.drawLine(okButtonX, okButtonY, okButtonX, okButtonY + okButtonHeight, 0x9FF3);
      ...
      tft.drawLine(saveButtonX, saveButtonY, saveButtonX + saveButtonWidth, saveButtonY, 0x9FF3);
      tft.drawLine(saveButtonX, saveButtonY, saveButtonX, saveButtonY + saveButtonHeight, 0x9FF3);
...
#ifndef FEATURE_TEXT
#define FEATURE_TEXT ACID_GREEN
#endif
...
static portMUX_TYPE batteryAdcMux = portMUX_INITIALIZER_UNLOCKED;

float readBatteryVoltage() {
  static bool batteryAdcConfigured = false;
  static float smoothed = 0.0f;

  if (!batteryAdcConfigured) {
    analogReadResolution(12);
    analogSetPinAttenuation(BATTERY_ADC_PIN, BATTERY_ADC_ATTEN);
    batteryAdcConfigured = true;
  }
...
#else
  const float dividerRatio = (BATTERY_VDIV_R1 + BATTERY_VDIV_R2) / BATTERY_VDIV_R2;
  float vBat = vAdc * dividerRatio;
#endif

  vBat *= BATTERY_CAL_FACTOR;

  if (smoothed <= 0.0f) {
    smoothed = vBat;
  } else {
    smoothed = smoothed * (1.0f - BATTERY_SMOOTHING) + vBat * BATTERY_SMOOTHING;
  }
...
static bool statusBarWifiRadioActive() {
  wifi_mode_t mode = WiFi.getMode();
  return mode == WIFI_MODE_STA || mode == WIFI_MODE_AP || mode == WIFI_MODE_APSTA;
}

static bool statusBarBleRadioActive() {
...
void updateSdCardStatus() {
  ...
  bool cardDetected = SDCardManager::refreshPresence();
...
void drawStatusBar(float batteryVoltage, bool forceUpdate) {
  String batteryVoltText = String(batteryVoltage, 2) + "V";
  int batteryCentivolts = (int)(batteryVoltage * 100.0f);
  logBatteryVoltagePeriodic();  static int lastTempBand = -1;

  static int lastBatteryPercentage = -1;
  static int lastBatteryCentivolts = -1;
  static int lastWifiDevices = -1;
  static int lastBleDevices = -1;
  static bool lastSdPresent = false;
  static int lastTempTenths = -1000;
  static bool lastWifiRadioActive = false;
  static bool lastBleRadioActive = false;

  int batteryPercentage = batteryDisplayPercentFromVoltage(batteryVoltage);
  batteryPercentage = constrain(batteryPercentage, 0, 100);

  int wifiDevices = WifiScan::getLastCount();
  int bleDevices  = BleScan::getLastCount();
  bool wifiScanUiActive = WifiScan::isUiScanningActive();
  bool bleScanUiActive  = BleScan::isUiScanningActive();
  float internalTemp = readInternalTemperature();
  int tempBand = (internalTemp >= 60.0f) ? 2 : ((internalTemp >= 45.0f) ? 1 : 0);
  uint8_t ledColorR = 0, ledColorG = 255, ledColorB = 0; // idle green
  // Priority: temperature alert -> SD missing -> WiFi scan -> BLE scan -> idle
  if (tempBand == 2) {
    ledColorR = 255; ledColorG = 0; ledColorB = 0;
  } else if (!sdCardPresent) {
    ledColorR = 255; ledColorG = 170; ledColorB = 0;
  } else if (wifiScanUiActive) {
    ledColorR = 255; ledColorG = 0; ledColorB = 255; // magenta
  } else if (bleScanUiActive) {
    ledColorR = 0; ledColorG = 180; ledColorB = 255;
  }
  int tempTenths = (int)(internalTemp * 10.0f);

  bool wifiRadioActive = statusBarWifiRadioActive();
  bool bleRadioActive  = statusBarBleRadioActive();

  if (!forceUpdate &&
      batteryPercentage == lastBatteryPercentage &&
      batteryCentivolts == lastBatteryCentivolts &&
      wifiDevices == lastWifiDevices &&
      bleDevices == lastBleDevices &&
      sdCardPresent == lastSdPresent &&
      tempTenths == lastTempTenths &&
      wifiRadioActive == lastWifiRadioActive &&
      bleRadioActive == lastBleRadioActive) {
      ledSetSimple(ledColorR, ledColorG, ledColorB);
    return;
  }
  ...
  tft.fillRect(0, 0, tft.width(), barHeight, STATUS_BG);
  tft.drawFastHLine(0, barHeight - 1, tft.width(), STATUS_DIVIDER);

  tft.drawRoundRect(batteryX, batteryY, 22, 10, 2, STATUS_TEXT);
  tft.fillRect(batteryX + 22, batteryY + 3, 2, 4, STATUS_TEXT);

  int batteryLevelWidth = ::map(batteryPercentage, 0, 100, 0, 18);
  batteryLevelWidth = constrain(batteryLevelWidth, 0, 18);

  uint16_t batteryColor =
    (batteryPercentage > 60) ? STATUS_ICON :
    (batteryPercentage > 20) ? STATUS_WARN_ICON :
                               STATUS_CRIT;

  tft.fillRect(batteryX + 2, batteryY + 2, 18, 6, STATUS_BG);
  tft.fillRect(batteryX + 2, batteryY + 2, batteryLevelWidth, 6, batteryColor);

  tft.setTextFont(1);
  tft.setTextSize(1);
  tft.setTextColor(STATUS_TEXT, STATUS_BG);
  tft.fillRect(batteryX + 29, batteryY + 2, 32, 8, STATUS_BG);
  tft.setCursor(batteryX + 29, batteryY + 2);
  tft.print(String(batteryPercentage) + "%");
  tft.fillRect(batteryX + 64, batteryY + 2, 42, 8, STATUS_BG);
  tft.setTextColor(STATUS_TEXT, STATUS_BG);
  tft.setCursor(batteryX + 64, batteryY + 2);
  tft.print(batteryVoltText);
  ...
  tft.drawBitmap(bleIconX, iconY, bitmap_icon_ble, 16, 16,
                 bleRadioActive ? STATUS_ACTIVE : STATUS_TEXT);

  uint16_t wifiColor = wifiRadioActive ? STATUS_ACTIVE : STATUS_TEXT;
  int wifiBaseY = 14;
  for (int i = 0; i < 4; i++) {
    int bh = (i + 1) * 3;
    int bw = 3;
    int bx = wifiBarsX + i * 5;
    if (wifiDevices > 0) {
      tft.fillRoundRect(bx, wifiBaseY - bh, bw, bh, 1, wifiColor);
    } else if (wifiRadioActive) {
      tft.drawRoundRect(bx, wifiBaseY - bh, bw, bh, 1, wifiColor);
    } else {
      tft.drawRoundRect(bx, wifiBaseY - bh, bw, bh, 1, STATUS_TEXT);
    }
  }

  uint16_t tempColor = (tempBand == 2) ? STATUS_CRIT : ((tempBand == 1) ? STATUS_WARN_ICON : STATUS_ICON);
  tft.drawBitmap(tempIconX, iconY, bitmap_icon_temp, 16, 16, tempColor);
  tft.fillRect(tempTextX, 3, 36, 10, STATUS_BG);
  tft.setTextColor(tempColor, STATUS_BG);
  tft.setCursor(tempTextX, 4);
  tft.print(String(internalTemp, 1));
  tft.print("C");

  if (sdCardPresent) {
    tft.drawBitmap(sdIconX, iconY, bitmap_icon_sdcard, 16, 16, STATUS_ICON);
  } else {
    tft.drawBitmap(sdIconX, iconY, bitmap_icon_nullsdcard, 16, 16, STATUS_WARN_ICON);
  }

  lastBatteryPercentage = batteryPercentage;
  lastBatteryCentivolts = batteryCentivolts;
  lastWifiDevices = wifiDevices;
  lastBleDevices = bleDevices;
  lastSdPresent = sdCardPresent;
  lastTempBand = tempBand;
  lastTempTenths = tempTenths;
  lastWifiRadioActive = wifiRadioActive;
  lastBleRadioActive = bleRadioActive;
...
static volatile bool statusBarSdPollingEnabled = true;
static volatile bool statusBarBatteryPollingEnabled = true;
static void statusBarTask(void* ) {
  for (;;) {
    // Keep this task extremely light to avoid stack overflows and mutex contention.
    // No TFT drawing here, no heavy SPI traffic while features/submenus are active.
    if (!feature_active && !in_sub_menu) {
      if (statusBarBatteryPollingEnabled) {
        currentBatteryVoltage = readBatteryVoltage();
      }
      // SD polling disabled from background task: update SD icon only from controlled UI paths.
      // if (statusBarSdPollingEnabled) updateSdCardStatus();
      statusBarDirty = true;
    }
...
void setStatusBarSdPollingEnabled(bool enabled) {
  statusBarSdPollingEnabled = enabled;
}

bool isStatusBarSdPollingEnabled() {
  return statusBarSdPollingEnabled;
}

void setStatusBarBatteryPollingEnabled(bool enabled) {
  statusBarBatteryPollingEnabled = enabled;
}

bool isStatusBarBatteryPollingEnabled() {
  return statusBarBatteryPollingEnabled;
}
...
void updateStatusBar() {
  ...
  if (currentMillis - lastStatusBarUpdate > 220) {
    float batteryVoltage = statusBarBatteryPollingEnabled ? currentBatteryVoltage : currentBatteryVoltage;
    if (fabs(batteryVoltage - lastBatteryVoltage) > 0.05 || lastBatteryVoltage == 0 || statusBarDirty) {
...
bool isSDCardAvailable() {
  ...
  return SDCardManager::ensureMounted();
}
...
void runUI() {
    ...
    if (!uiDrawn) {
        ...
        tft.drawLine(0, STATUS_BAR_Y_OFFSET + STATUS_BAR_HEIGHT, SCREEN_WIDTH, STATUS_BAR_Y_OFFSET + STATUS_BAR_HEIGHT, ACID_GREEN);
        uiDrawn = true;
    }
            ...
            switch (activeIcon) {
                case 0:
                  if (terminalActive) {
                    ...
                    tft.fillRect(0, 37, DISPLAY_WIDTH, 16, ACID_GREEN);
...
void logBatteryVoltagePeriodic() {
  ...
  int rawPct = batteryPercentFromVoltage(vbat);
  int dispPct = batteryDisplayPercentFromVoltage(vbat);
  ...
  Serial.print(rawPct);
  Serial.print("% | ui=");
  Serial.print(dispPct);
  Serial.println("%");
}

int batteryPercentFromVoltage(float v) {
  if (v <= BATTERY_FORCE_EMPTY_VOLTAGE) return 0;
  if (v >= BATTERY_FORCE_FULL_VOLTAGE) return 100;

  // Li-ion 1S practical curve under light/medium load.
  // Tuned to avoid pessimistic readings when display/radio load causes small voltage sag.
  if (v >= 3.80f) return 100;
  if (v >= 3.75f) return 90;
  if (v >= 3.70f) return 80;
  if (v >= 3.65f) return 70;
  if (v >= 3.60f) return 60;
  if (v >= 3.55f) return 50;
  if (v >= 3.50f) return 40;
  if (v >= 3.45f) return 35;
  if (v >= 3.40f) return 30;
  if (v >= 3.35f) return 20;
...
int batteryDisplayPercentFromVoltage(float v) {
  int target = batteryPercentFromVoltage(v);

  // Hard clamps: these must be immediate and truthful.
  if (v <= BATTERY_FORCE_EMPTY_VOLTAGE) return 0;
  if (v >= BATTERY_FORCE_FULL_VOLTAGE) return 100;

  static bool initialized = false;
  static float smoothPercent = 0.0f;
  static int lastShown = -1;

  if (!initialized) {
    smoothPercent = target;
    lastShown = target;
    initialized = true;
    return target;
  }

  smoothPercent = smoothPercent * (1.0f - BATTERY_PERCENT_SMOOTHING) + target * BATTERY_PERCENT_SMOOTHING;

  int rounded = (int)(smoothPercent + 0.5f);

  // Hysteresis: avoid 1% flicker from small ADC/load oscillations.
  if (lastShown >= 0 && abs(rounded - lastShown) <= BATTERY_PERCENT_HYSTERESIS) {
    return lastShown;
  }
...
int getBatteryPercentage() {
  float vbat = readBatteryVoltage();
  return batteryDisplayPercentFromVoltage(vbat);
}
