...
#endif
void radioDebugLogMode(const char* modeName);
#define RADIO_IDLE   0
#define RADIO_WIFI   1
#define RADIO_BLE    2
#define RADIO_SUBGHZ 3
#define RADIO_RF24   4
bool radioStateTryAcquire(int newState, uint32_t settleMs = 120);
void radioStateReleaseToIdle(uint32_t settleMs = 80);
int radioStateCurrentOwner();
bool radioStateIsBusy();
uint32_t radioStateOwnerAgeMs();
void radioDebugLogMode(const char* modeName);
#define RDLOG(msg) do { Serial.println(msg); } while(0)
#define RDLOGF(label, value) do { Serial.print(label); Serial.println(value); } while(0)
namespace BleScan { void pauseForWifiFeature(); void resumeAfterWifiFeature(); void notifyWifiScanStarted(); void notifyWifiScanFinished(); }
namespace BleScan { void pauseForWifiFeature(); void resumeAfterWifiFeature(); }




static void prepareAggressiveWifiScan() {
...
static int performAggressiveWifiScan() {
  prepareAggressiveWifiScan();

  wifi_scan_config_t scan_cfg = {};
  scan_cfg.ssid = nullptr;
  scan_cfg.bssid = nullptr;
  scan_cfg.channel = 0;         // all channels
  scan_cfg.show_hidden = true;  // closer to Arduino test scanner behavior
  scan_cfg.scan_type = WIFI_SCAN_TYPE_ACTIVE;
  scan_cfg.scan_time.active.min = 120;
  scan_cfg.scan_time.active.max = 260;

  // Use esp-idf scan directly for more complete results on ESP32-S3.
...
static bool wifiFeatureDriverReady = false;
...
static void prepareWifiFeatureDriver() {
  ...
  // Conservative path: do not recreate netif/STA on feature entry.
  // Just reuse the current WiFi driver state.
  WiFi.setSleep(true);
  WiFi.disconnect(false, false);
  delay(60);
  wifiFeatureDriverReady = true;
}

static void cleanupWifiFeatureDriver() {
  WiFi.scanDelete();
  esp_wifi_set_promiscuous(false);
  delay(20);
  WiFi.disconnect(false, false);
...
static void cleanupPacketMonitorForScanner() {
  esp_wifi_set_promiscuous(false);
  delay(40);
  WiFi.scanDelete();
  delay(20);
...
static void preparePacketMonitorWifi() {
  // Keep helper only for cleanup symmetry; Packet Monitor setup no longer forces WIFI_MODE_NULL.
  WiFi.scanDelete();
  esp_wifi_set_promiscuous(false);
  delay(30);
}

static void cleanupPacketMonitorWifi() {
  esp_wifi_set_promiscuous(false);
  delay(30);
  WiFi.scanDelete();
  delay(20);
...
void ptmSetup() {
  ...
  // Packet Monitor needs the radio active to see packets.
  esp_wifi_set_promiscuous(false);
  delay(20);
  if (WiFi.getMode() != WIFI_MODE_STA) {
    WiFi.mode(WIFI_STA);
...
static volatile uint32_t mgmtBeaconCount = 0;
static volatile uint32_t mgmtProbeReqCount = 0;
static volatile uint32_t mgmtProbeRespCount = 0;
static volatile uint32_t mgmtAssocReqCount = 0;
static volatile uint32_t mgmtAssocRespCount = 0;
static volatile uint32_t mgmtAuthCount = 0;
static volatile uint32_t mgmtDeauthCount = 0;
static volatile uint32_t mgmtDisassocCount = 0;
static volatile uint32_t mgmtOtherCount = 0;
static volatile uint32_t mgmtLastDebugMs = 0;
static volatile int pendingDeauthIndex = -1;
static volatile bool pendingDeauthNotice = false;
static volatile uint8_t monitorChannel = 0;
static bool channelHopEnabled = true;
static uint8_t hopChannels[13] = {0};
static uint8_t hopChannelCount = 0;
static uint8_t hopIndex = 0;
static uint32_t lastHopMs = 0;
static uint16_t hopDwellMs = DEAUTH_HOP_DWELL_MS;

static void resetDeauthDebugCounters() {
  mgmtBeaconCount = 0;
  mgmtProbeReqCount = 0;
  mgmtProbeRespCount = 0;
  mgmtAssocReqCount = 0;
  mgmtAssocRespCount = 0;
  mgmtAuthCount = 0;
  mgmtDeauthCount = 0;
  mgmtDisassocCount = 0;
  mgmtOtherCount = 0;
  pendingDeauthIndex = -1;
  pendingDeauthNotice = false;
  monitorChannel = 0;
  ...
  lastHopMs = 0;
  hopDwellMs = 220;
  for (int i = 0; i < 13; i++) hopChannels[i] = 0;
  mgmtLastDebugMs = millis();
}

static void printDeauthDebugCounters() {
  uint8_t ch = 0;
  wifi_second_chan_t second = WIFI_SECOND_CHAN_NONE;
  esp_wifi_get_channel(&ch, &second);
  Serial.printf("[DeauthDetector] ch=%u hop=%u/%u dwell=%u beacon=%lu probeReq=%lu probeResp=%lu assocReq=%lu assocResp=%lu auth=%lu disassoc=%lu deauth=%lu other=%lu\n",
    (unsigned)ch,
    (unsigned)(hopChannelCount ? (hopIndex + 1) : 0),
    (unsigned)hopChannelCount,
    (unsigned)hopDwellMs,
    (unsigned long)mgmtBeaconCount,
    (unsigned long)mgmtProbeReqCount,
    (unsigned long)mgmtProbeRespCount,
    (unsigned long)mgmtAssocReqCount,
    (unsigned long)mgmtAssocRespCount,
    (unsigned long)mgmtAuthCount,
    (unsigned long)mgmtDisassocCount,
    (unsigned long)mgmtDeauthCount,
    (unsigned long)mgmtOtherCount);
}
...
void checkButtonPress() {
  if (!pcf.digitalRead(BTN_UP)) {
    ...
    if (!stopScan) {
      stopScan = true;
      if (tftSemaphore != NULL && xSemaphoreTake(tftSemaphore, pdMS_TO_TICKS(50)) == pdTRUE) {
...
void snifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  if (stopScan || exitMode || buf == NULL) return;
  ...
  if (type == WIFI_PKT_MGMT) {
    ...
    switch (frameSubtype) {
      case 0x08: mgmtBeaconCount++; break;
      case 0x04: mgmtProbeReqCount++; break;
      case 0x05: mgmtProbeRespCount++; break;
      case 0x00: mgmtAssocReqCount++; break;
      case 0x01: mgmtAssocRespCount++; break;
      case 0x0B: mgmtAuthCount++; break;
      case 0x0A: mgmtDisassocCount++; break;
      case 0x0C: {
        mgmtDeauthCount++;
        uint8_t senderMAC[6];
        ...
        for (int i = 0; i < MAX_NETWORKS; i++) {
          if (ssidLists[i].length() == 0) continue;
          if (memcmp(senderMAC, macList[i], 6) == 0) {
            deauth[i]++;
            pendingDeauthIndex = i;
            pendingDeauthNotice = true;
            break;
          }
        }
        break;
      }
      default: mgmtOtherCount++; break;
    }
  }
}
...
void scanWiFiTask(void *param) {
  ...
  if (tftSemaphore != NULL && xSemaphoreTake(tftSemaphore, pdMS_TO_TICKS(50)) == pdTRUE) {
    displayPrint("[*] Initial WiFi scan for detector", TFT_CYAN, true);
    xSemaphoreGive(tftSemaphore);
  }

  int n = performAggressiveWifiScan();
  int bestIdx = -1;
  int bestRssi = -1000;
  ...
  // True multichannel mode across the whole 2.4 GHz band.
  hopChannelCount = 13;
  for (int i = 0; i < 13; i++) {
    hopChannels[i] = (uint8_t)(i + 1);
  }

  if (bestIdx >= 0) {
    monitorChannel = (uint8_t)WiFi.channel(bestIdx);
    hopIndex = (monitorChannel >= 1 && monitorChannel <= 13) ? (uint8_t)(monitorChannel - 1) : 0;
    ...
    if (tftSemaphore != NULL && xSemaphoreTake(tftSemaphore, pdMS_TO_TICKS(50)) == pdTRUE) {
      displayPrint("[*] Detector armed, multichannel", TFT_GREEN, true);
      displayPrint("[*] Start CH " + String(hopChannels[hopIndex]) + " / " + String(hopChannelCount) + " ch", TFT_GREEN, true);
      xSemaphoreGive(tftSemaphore);
    }
    Serial.printf("[DeauthDetector] armed multichannel, start channel %u, hopCount=%u, dwell=%u ms, promiscuous ON\n",
                  (unsigned)hopChannels[hopIndex], (unsigned)hopChannelCount, (unsigned)hopDwellMs);
  } else {
    if (tftSemaphore != NULL && xSemaphoreTake(tftSemaphore, pdMS_TO_TICKS(50)) == pdTRUE) {
      displayPrint("[!] No AP found for channel lock", UI_WARN, true);
      xSemaphoreGive(tftSemaphore);
    }
  }
...
void uiTask(void *param) {
  while (1) {
    if (stopScan || exitMode || tftSemaphore == NULL) break;

    if (xSemaphoreTake(tftSemaphore, pdMS_TO_TICKS(50)) == pdTRUE) {
      runUI();
      xSemaphoreGive(tftSemaphore);
    }
...
void deauthdetectSetup() {
  ...
  WiFi.disconnect(false, false);
  delay(80);
  resetDeauthDebugCounters();
  Serial.println("[DeauthDetector] debug counters reset");
  Serial.println("[DeauthDetector] touch dwell controls: top - / + buttons");
  prepareWifiFeatureDriver();
  WiFi.disconnect();
...
void deauthdetectLoop() {
  static uint32_t lastCoexYieldMs = 0;

  if (channelHopEnabled && hopChannelCount > 1 && !stopScan && !exitMode) {
    uint32_t now = millis();
    if ((now - lastHopMs) > hopDwellMs) {
      hopIndex = (uint8_t)((hopIndex + 1) % hopChannelCount);
      esp_wifi_set_channel(hopChannels[hopIndex], WIFI_SECOND_CHAN_NONE);
      lastHopMs = now;
    }
  }

  if ((millis() - mgmtLastDebugMs) > 2000) {
    printDeauthDebugCounters();
    mgmtLastDebugMs = millis();
  }

  if (pendingDeauthNotice && pendingDeauthIndex >= 0 && pendingDeauthIndex < MAX_NETWORKS) {
    if (tftSemaphore != NULL && xSemaphoreTake(tftSemaphore, pdMS_TO_TICKS(50)) == pdTRUE) {
      displayPrint("[!] Deauth Attack on: " + ssidLists[pendingDeauthIndex], UI_WARN, true);
      xSemaphoreGive(tftSemaphore);
    }
    pendingDeauthNotice = false;
    pendingDeauthIndex = -1;
  ...
  if ((millis() - lastCoexYieldMs) > RADIO_COEX_YIELD_MS) {
    delay(1);
    lastCoexYieldMs = millis();
  }
  ...
  if (stopScan || exitMode) {
    ...
    // Graceful shutdown: let worker tasks exit by themselves.
    // Do NOT delete tasks here while they may still own tftSemaphore.
    esp_wifi_set_promiscuous(false);
    esp_wifi_set_promiscuous_rx_cb(NULL);
    delay(20);
    WiFi.disconnect(false, false);
    ...
    while ((wifiScanTaskHandle != NULL) && (millis() - waitStart < 1200)) {
      delay(20);
    }

    // Leave the TFT semaphore alive. Deleting it here can race with UI/status tasks
    // and trigger xQueueGenericSend asserts on exit.
    stopScan = false;
    exitMode = false;
    lineIndex = 0;
    uiDrawn = false;
  ...
  for (;;) {
    ...
    if (settings().autoWifiScan && idleOk && !feature_active && !in_sub_menu) {
      ...
      if (!bgScanRunning) {
        ...
        WiFi.scanDelete();
  delay(60);
        int ret = performAggressiveWifiScan();
  ...
  if (fullRedraw || networkCount <= 0) {
    ...
    for (int i = 0; i < MAX_VISIBLE_ITEMS; i++) {
      ...
      if (currentNetworkIndex == currentIndex) {
        tft.setTextColor(STATUS_ICON, TFT_BLACK);
...
void startWiFiScan() {
  ...
  listStartIndex = 0;
  isDetailView = false;
  RDLOG("[WIFI] startWiFiScan");
  ::BleScan::notifyWifiScanStarted();
  ::BleScan::pauseForWifiFeature();
  WiFi.setSleep(true);
  ledSetExternal(255, 0, 255);
  displayScanning();
  ...
  uint32_t wifiScanT0 = millis();
  int numNetworks = performAggressiveWifiScan();
  LOGPV("[WIFI]", "networks=", numNetworks);
  LOGPV("[WIFI]", "scan_ms=", (millis() - wifiScanT0));
  RDLOGF("[WIFI] networks=", numNetworks);

  isScanning = false;
  ::BleScan::notifyWifiScanFinished();
  ::BleScan::resumeAfterWifiFeature();
  ledSetExternal(0, 255, 0);
...
void wifiscanSetup() {
  ...
  cleanupPacketMonitorForScanner();
...
void wifiscanLoop() {
  static uint32_t uiFrameMs = 0;
  uint32_t nowMs = millis();
  if ((nowMs - uiFrameMs) < UI_FRAME_LOCK_MS) return;
  uiFrameMs = nowMs;
...
static bool cpCloneScanAndSelect(String& outSsid, uint8_t& outChannel, uint8_t outBssid[6]) {
  ...
  while (true) {
    ...
    loading(90, STATUS_ICON, 0, 0, 2, true);
  ...
  delay(60);
    ...
    while (true) {
      ...
      for (int row = start; row < end; row++) {
        ...
        uint16_t color = (auth == WIFI_AUTH_OPEN) ? STATUS_ICON : TFT_WHITE;
        tft.setTextColor(color, TFT_BLACK);
        tft.setCursor(8, y);
        tft.println(buf);
        y += 16;
...
void drawMainMenu() {
  ...
  if (cp_deauth_active) {
    ...
    displayPrint(buf, STATUS_ICON, false);
  }
}

void drawInputField() {
  tft.fillRect(10, 55, 220, 25, TFT_DARKGREY);
  tft.drawRect(9, 54, 222, 27, STATUS_ICON);
  tft.setTextColor(TFT_WHITE);
  tft.setTextSize(2);
  tft.setCursor(15, 60);
...
tft.drawRoundRect(165, 185, 70, 25, 4, STATUS_ICON);
...
int dedup_ap_records_by_bssid(wifi_ap_record_t *records, int count) {
    if (!records || count <= 1) return count;

    int out = 0;
    for (int i = 0; i < count; ++i) {
        bool duplicate = false;
        for (int j = 0; j < out; ++j) {
            if (memcmp(records[i].bssid, records[j].bssid, 6) == 0) {
                duplicate = true;
                // keep stronger RSSI if the duplicate is better
    ...
    qsort(records, out, sizeof(wifi_ap_record_t), compare_ap);
    return out;
}
...
void drawScanScreen() {
    ...
    if (scanning) {
        ...
        loading(100, STATUS_ICON, 0, 0, 3, true);
        tft.setCursor(10, 65);
    ...
    if (network_count == 0) {
        ...
        for (int i = start_index; i < end_index && y < 300; i++) {
            ...
            tft.setCursor(10, y);
            tft.setTextColor(i == selected_ap_index ? STATUS_ICON : (ap_list[i].authmode == WIFI_AUTH_OPEN ? STATUS_ICON : WHITE));
            tft.println(buf);
            y += 15;
        }
...
bool scanNetworks() {
    ...
    network_count = performAggressiveWifiScan();
    if (network_count == 0) {
        scanning = false;
        ::BleScan::notifyWifiScanFinished();
        ::BleScan::resumeAfterWifiFeature();
    ...
    network_count = dedup_ap_records_by_bssid(ap_list, network_count);
    current_page = 0;
...
void drawAttackScreen() {
    ...
    tft.setTextColor(attack_running ? STATUS_ICON : DARK_GRAY);
  ...
  while (!selected) {
    ...
    for (int i = startIndex; i < end_index && y_pos < 300; i++) {
      if (x >= 10 && x < SCREEN_WIDTH - 10 && y >= y_pos && y < y_pos + NETWORK_ROW_HEIGHT) {
        ...
        tft.println(buf);
        delay(100);
        tft.setTextColor(i == selectedIndex ? STATUS_ICON : (networks[i].authmode == WIFI_AUTH_OPEN ? STATUS_ICON : TFT_WHITE), TFT_BLACK);
        tft.setCursor(10, y_pos);
        tft.println(buf);
        selectedIndex = i;
...
void drawNetworkList(int startIndex, int numNetworks, NetworkInfo* networks, int selectedIndex) {
  ...
  if (numNetworks == 0) {
    ...
    for (int i = start_index; i < end_index && y < 300; i++) {
      ...
      snprintf(buf, sizeof(buf), "%02d: %-15s %3d dBm Ch%2d %s", i + 1, ssid, networks[i].rssi, networks[i].channel, enc);
      tft.setCursor(10, y);
      tft.setTextColor(i == selectedIndex ? STATUS_ICON : (networks[i].authmode == WIFI_AUTH_OPEN ? STATUS_ICON : TFT_WHITE));
      tft.println(buf);
...
void powerMgrStopWifiActivity() {
  LOGP("[WIFI]", "power stop");
  RDLOG("[WIFI] powerMgrStopWifiActivity");
  esp_wifi_set_promiscuous(false);
  WiFi.scanDelete();
