...
static uint16_t uiElegantSafeSelectionColor() {
    return TFT_GREENYELLOW;
}

static uint16_t uiElegantSafeNormalColor() {
    return TFT_WHITE;
}

static uint16_t uiElegantSafeDimColor() {
    return TFT_DARKGREY;
}


#define LOGGING_PRO_ENABLED 1

#if LOGGING_PRO_ENABLED
...
#else
  #define LOGP(tag, msg) do {} while(0)
  #define LOGPV(tag, label, value) do {} while(0)
#endif

#define RDLOG(msg) LOGP("[RADIO]", msg)
#define RDLOGF(label, value) LOGPV("[RADIO]", label, value)

void radioDebugLogMode(const char* modeName) {
    LOGPV("[MODE]", "mode=", modeName);
}



// ===== STATE MANAGER PRO HARD =====
#define STATE_MANAGER_PRO_HARD 1
...
static volatile int gRadioOwner = RADIO_IDLE;
static volatile uint32_t gRadioOwnerSinceMs = 0;
static volatile bool gRadioTransitionBusy = false;

bool radioStateTryAcquire(int newState, uint32_t settleMs = 120) {
    if (gRadioTransitionBusy) {
        RDLOG("[STATE] busy");
        return false;
    }

    gRadioTransitionBusy = true;

    if (gRadioOwner == newState) {
        gRadioTransitionBusy = false;
        return true;
    }

    RDLOG("[STATE] switching");

    switch (gRadioOwner) {
        case RADIO_WIFI: RDLOG("[STATE] release WIFI"); break;
        case RADIO_BLE: RDLOG("[STATE] release BLE"); break;
        case RADIO_RF24: RDLOG("[STATE] release RF24"); break;
        case RADIO_SUBGHZ: RDLOG("[STATE] release SUBGHZ"); break;
        case RADIO_IDLE: break;
    }

    delay(settleMs);

    gRadioOwner = newState;
    gRadioOwnerSinceMs = millis();

    switch (newState) {
        case RADIO_WIFI: RDLOG("[STATE] acquire WIFI"); break;
        case RADIO_BLE: RDLOG("[STATE] acquire BLE"); break;
        case RADIO_RF24: RDLOG("[STATE] acquire RF24"); break;
        case RADIO_SUBGHZ: RDLOG("[STATE] acquire SUBGHZ"); break;
        default: RDLOG("[STATE] acquire IDLE"); break;
    }
...
void radioStateReleaseToIdle(uint32_t settleMs = 80) {
    if (gRadioTransitionBusy) return;
    gRadioTransitionBusy = true;
    if (gRadioOwner != RADIO_IDLE) {
        RDLOG("[STATE] release->IDLE");
        delay(settleMs);
        gRadioOwner = RADIO_IDLE;
        gRadioOwnerSinceMs = millis();
    }
    gRadioTransitionBusy = false;
}
...
bool radioStateIsBusy() {
    return gRadioTransitionBusy;
}

uint32_t radioStateOwnerAgeMs() {
    return millis() - gRadioOwnerSinceMs;
}
// ===== END STATE MANAGER PRO HARD =====


void e01InitHardeningPreIdleAllSpi() {
...
void e01InitHardeningPrepareRadio1() {
    digitalWrite(CE_PIN_3, LOW);
    digitalWrite(CSN_PIN_3, HIGH);
    digitalWrite(CE_PIN_1, LOW);
    digitalWrite(CSN_PIN_1, HIGH);
...
void e01InitHardeningPrepareRadio3() {
    digitalWrite(CE_PIN_1, LOW);
    digitalWrite(CSN_PIN_1, HIGH);
    digitalWrite(CE_PIN_3, LOW);
    digitalWrite(CSN_PIN_3, HIGH);
...
namespace WifiScan { void powerMgrStopWifiActivity(); }
namespace BleScan { void powerMgrStopBleActivity(); void powerMgrResumeBleIdle(); }
namespace BleSniffer { void powerMgrStopBleSniffer(); }
namespace replayat { void powerMgrStopSubGhzActivity(); }
...
static PowerMode gPowerMode = PowerMode::IDLE;

void powerMgrEnterIdleMode() {
    radioDebugLogMode("IDLE");
    WifiScan::powerMgrStopWifiActivity();
    BleScan::powerMgrStopBleActivity();
    BleSniffer::powerMgrStopBleSniffer();
    replayat::powerMgrStopSubGhzActivity();
    BleScan::powerMgrResumeBleIdle();
    gPowerMode = PowerMode::IDLE;
}

void powerMgrEnterWifiMode() {
    radioDebugLogMode("WIFI_ACTIVE");
    BleScan::powerMgrStopBleActivity();
    BleSniffer::powerMgrStopBleSniffer();
    replayat::powerMgrStopSubGhzActivity();
    gPowerMode = PowerMode::WIFI_ACTIVE;
}

void powerMgrEnterBleMode() {
    radioDebugLogMode("BLE_ACTIVE");
    WifiScan::powerMgrStopWifiActivity();
    replayat::powerMgrStopSubGhzActivity();
    gPowerMode = PowerMode::BLE_ACTIVE;
}

void powerMgrEnterSubGhzMode() {
    radioDebugLogMode("SUBGHZ_ACTIVE");
    WifiScan::powerMgrStopWifiActivity();
    BleScan::powerMgrStopBleActivity();
    BleSniffer::powerMgrStopBleSniffer();
    gPowerMode = PowerMode::SUBGHZ_ACTIVE;
}
...
void setup() {
    LOGP("[BOOT]", "setup begin");
    e01InitHardeningPreIdleAllSpi();
    e01PowerMgrSleepAll();
...
void uiReturnToMainMenuClean() {
    powerMgrEnterIdleMode();
    in_sub_menu = false;
    is_main_menu = true;
    submenu_initialized = false;
    menu_initialized = false;
