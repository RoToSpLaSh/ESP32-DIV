...
float readBatteryVoltage() {
  portENTER_CRITICAL(&batteryAdcMux);
  uint16_t raw = analogRead(BATTERY_ADC_PIN);
  portEXIT_CRITICAL(&batteryAdcMux);
...
void drawStatusBar(float batteryVoltage, bool forceUpdate) {  static int lastTempBand = -1;
  ...
  int batteryPercentage = ::map((int)(batteryVoltage * 100.0f), 300, 420, 0, 100);
