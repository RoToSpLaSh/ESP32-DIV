...
static SPIClass irSavedSdSpi(FSPI);
...
static bool irPrepareAndMountSd() {
  irPrepareSdBus();

  irSavedSdSpi.end();
  delay(5);
  irSavedSdSpi.begin(SD_SCLK, SD_MISO, SD_MOSI, SD_CS);
  delay(5);

  bool ok = SD.begin(SD_CS, irSavedSdSpi, 8000000);
  if (!ok) {
    delay(20);
    ok = SD.begin(SD_CS, irSavedSdSpi, 4000000);
  }
  return ok;
}
