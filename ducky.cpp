...
static SPIClass duckySdSpi(FSPI);


static void duckyPrepareSdBus() {
...
static bool mountSD() {
  ...
  duckySdSpi.end();
  delay(5);
  duckySdSpi.begin(SD_SCLK, SD_MISO, SD_MOSI, SD_CS);
  delay(5);

  sd_mounted = SD.begin(SD_CS, duckySdSpi, 8000000);
  if (!sd_mounted) {
    delay(20);
    sd_mounted = SD.begin(SD_CS, duckySdSpi, 4000000);
  }
  return sd_mounted;
}
static bool ensureDuckyDir() {
  ...
  if (SD.exists(DUCKY_DIR)) {
    sd_mounted = true;
    return true;
  }
  sd_mounted = SD.mkdir(DUCKY_DIR);
  return sd_mounted;
}

static bool duckyRefreshSdStatus() {
  bool ok = ensureDuckyDir();
  sd_mounted = ok;
  return ok;
}
...
void enter() {
  bool sdOk = duckyRefreshSdStatus();
  if (!sdOk) {
    delay(50);
    sdOk = duckyRefreshSdStatus();
  }
  sd_mounted = sdOk;
  ...
  if (!sdOk) {
    ...
    tft.setTextFont(1);
    tft.setTextColor(UI_TEXT, COL_BG);
    tft.drawCentreString("Retry from menu", DISPLAY_WIDTH / 2, DISPLAY_HEIGHT / 2 + 10, 1);
    delay(900);
    ducky_active = false;
    feature_exit_requested = true;
    return;
  ...
  if (!hid) {
    ...
    msd.push_back(0x00);
    msd.push_back(0x00);
    msd.push_back((char)0x80);
    advData.setManufacturerData(msd);
    NimBLEAdvertisementData scanRsp;
    scanRsp.setName(DEV_NAME);
