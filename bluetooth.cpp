...
static void prepareE01SharedBus() {
...
void updateSpoofer() {
  tft.drawLine(0, 292, 240, 292, STATUS_ICON);
...
void runUI() {
  ...
  if (animationState > 0 && millis() - lastAnimationTime >= 150) {
    if (animationState == 1) {
      ...
      switch (activeIcon) {
        case 0: feature_active=false; in_sub_menu=false; feature_exit_requested = true; break;
      }
    } else if (animationState == 2) {
      animationState = 0;
      activeIcon = -1;
...
void rf24WakeWarmup() {
  LOGP("[RF24]", "wake warmup");
  RDLOG("[RF24] wakeWarmup");
  prepareE01SharedBus();
  delay(20);
}

void rf24PowerDownAllReal() {
  LOGP("[RF24]", "powerDownAllReal");
  RDLOG("[RF24] powerDownAllReal");
...
void initializeRadiosMultiMode() {
  ...
  RDLOG("[RF24] radio1 begin attempt");
  LOGP("[RF24]", "radio1 begin attempt");
  if (radio1.begin()) {
    configureRadio(radio1, channelGroup1, sizeof(channelGroup1));
    radio1Active = true;
    LOGP("[RF24]", "radio1 init OK");
    RDLOG("[RF24] radio1 init OK");
  }
  RDLOG("[RF24] radio2 begin attempt");
  LOGP("[RF24]", "radio2 begin attempt");
  if (radio2.begin()) {
    configureRadio(radio2, channelGroup2, sizeof(channelGroup2));
    radio2Active = true;
    LOGP("[RF24]", "radio2 init OK");
    RDLOG("[RF24] radio2 init OK");
  }
  RDLOG("[RF24] radio3 begin attempt");
  LOGP("[RF24]", "radio3 begin attempt");
  if (radio3.begin()) {
    configureRadio(radio3, channelGroup3, sizeof(channelGroup3));
    radio3Active = true;
    LOGP("[RF24]", "radio3 init OK");
    RDLOG("[RF24] radio3 init OK");
  }
}
...
static volatile uint32_t lastWifiScanStartMs = 0;
static volatile uint32_t lastWifiScanEndMs = 0;
static const uint32_t RADIO_SCHEDULER_BLE_HOLDOFF_AFTER_WIFI_MS = 1400;

void notifyWifiScanStarted();
void notifyWifiScanFinished();
bool bleSchedulerAllowsBackgroundScan();

void stopBackgroundScanner();
void pauseForExternalBleFeature();
void resumeIdleBackgroundScanner();
...
void notifyWifiScanFinished() {
  lastWifiScanEndMs = millis();
}


bool bleSchedulerWifiQuietWindow() {
  uint32_t now = millis();
  return (now - lastWifiScanEndMs) >= RADIO_SCHEDULER_BLE_HOLDOFF_AFTER_WIFI_MS;
}
...
static const uint32_t BG_BLE_SCAN_INTERVAL_MS = 3200;
static bool bleInitDone = false;

static void hardResetBleScannerState() {
  isScanning = false;
...
static void ensureBleInit() {
  ...
  bleScan->setInterval(220);
  bleScan->setWindow(190);
  bleInitDone = true;
}
...
static void bgBleScanTask(void* ) {
  ...
  for (;;) {
    ...
    if (settings().autoBleScan && idleOk && !feature_active && !in_sub_menu) {
      if (!bleSchedulerWifiQuietWindow()) {
        vTaskDelay(180 / portTICK_PERIOD_MS);
        continue;
      }

      if ((now - bgLastScanMs) < BG_BLE_SCAN_INTERVAL_MS) {
        vTaskDelay(180 / portTICK_PERIOD_MS);
        continue;
      }
      ...
      bleScan->stop();
      bleScan->clearResults();
      bleResults = bleScan->start(4, true);
      isScanning = false;
...
void startBLEScan() {
  ...
  bleScan->stop();
  uint32_t bleScanT0 = millis();
  bleScan->clearResults();
  bleResults = bleScan->start(8, true);
  LOGPV("[BLE]", "scan_ms=", (millis() - bleScanT0));
  LOGPV("[BLE]", "devices=", bleResults.getCount());
  isScanning = false;
  screenNeedsUpdate = true;
...
void updateBLEList() {
  ...
  if (deviceCount > 0) {
    ...
    if (listStartIndex > (deviceCount - 1)) listStartIndex = deviceCount - 1;
  } else {
    currentIndex = 0;
    listStartIndex = 0;
  }
  ...
  deviceCount = bleResults.getCount();
  if (deviceCount <= 0) {
    if (fullScreenUpdate) {
...
void bleScanSetup() {
  LOGP("[BLE]", "scanner setup");
  if (!radioStateTryAcquire(RADIO_BLE, 140)) return;
  RDLOG("[BLE] Scanner setup");
  tft.fillScreen(TFT_BLACK);
  delay(30);

  stopBackgroundScanner();

  hardResetBleScannerState();
  ensureBleInit();
  tft.fillScreen(TFT_BLACK);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setTextSize(1);
...
void stopBackgroundScanner() {
  LOGP("[BLE]", "bg stop");
  RDLOG("[BLE] stopBackgroundScanner");
  if (bgBleScanTaskHandle != nullptr) {
    vTaskDelete(bgBleScanTaskHandle);
    bgBleScanTaskHandle = nullptr;
    delay(120);
  }
}

void pauseForExternalBleFeature() {
  LOGP("[BLE]", "pause external feature");
  RDLOG("[BLE] pauseForExternalBleFeature");
  stopBackgroundScanner();
...
void resumeIdleBackgroundScanner() {
  LOGP("[BLE]", "bg resume idle");
  RDLOG("[BLE] resumeIdleBackgroundScanner");
  if (settings().autoBleScan && bgBleScanTaskHandle == nullptr) {
    startBackgroundScanner();
  }
}

void pauseForWifiFeature() {
  pauseForExternalBleFeature();
}

void resumeAfterWifiFeature() {
  resumeIdleBackgroundScanner();
}
...
void scannerRf24PrepareEntry() {
  LOGP("[RF24]", "scanner prepare entry");
  RDLOG("[RF24] scanner prepare entry");
  prepareE01SharedBus();
...
void calibrateBackgroundNoise() {
  ...
  Print("[!] Calibrating background noise", STATUS_ICON, false);
...
void scan() {
  Print("[!] Refresh Scanner.", STATUS_ICON, false);
  disable();
  for (int j = 0; j < 50; j++) {
    for (int i = 0; i < CHANNELS; i++) {
...
void scannerSetup() {
  ...
  SPI.end();
  delay(2);
  SPI.begin(CC1101_SCK, CC1101_MISO, CC1101_MOSI, CSN_PIN_1);
...
static void hardResetBleSnifferEngine() {
  BLEDevice::deinit(true);
  delay(220);
}
...
private:
  ...
  void updateHeader() {
    ...
    tft.drawLine(0, 56, 240, 56, STATUS_ICON);
  }
  ...
  void updateDisplay() {
    ...
    for (int i = 0; i < MAX_LINES; i++) {
      ...
      if (displayLines[i].originalColor == STATUS_ICON && !displayLines[i].isAlert) {
        tft.drawRect(3, y, tft.width() - 6, LINE_HEIGHT - 2, STATUS_ICON);
      }
      ...
      if (!device.isSuspicious) {
        ...
        if (device.display && !device.jammingAlerted) {
          ...
          addLine(String(lineNumber++) + " -> Jamming Suspected (" + protocol + "): " + device.mac + " T:" + String(timestamp),
                  STATUS_ICON, true, MessageType::ALERT);
  ...
  void processNewDevice(BLEAdvertisedDevice* bleDevice, esp_bt_gap_cb_param_t* btDevice, unsigned long timestamp, bool isBLE) {
    ...
    if (device.display) {
      ...
      addLine(line, device.isSuspicious ? STATUS_ICON : GREEN, false, MessageType::DEVICE);
    }
    deviceCount++;
  ...
  void refreshDisplay() {
    ...
    for (int i = 0; i < deviceCount; i++) {
      if (devices[i].display) {
        ...
        addLine(line, devices[i].isSuspicious ? STATUS_ICON : GREEN, false, MessageType::DEVICE);
      }
    }
  }
...
void runUI() {
  ...
  if (!uiDrawn) {
    tft.drawLine(0, 19, 240, 19, TFT_WHITE);
    tft.drawLine(0, 36, 240, 36, STATUS_ICON);
    tft.fillRect(0, STATUS_BAR_Y_OFFSET, SCREEN_WIDTH, STATUS_BAR_HEIGHT, DARK_GRAY);
...
public:
  ...
  void resetState() {
    ...
    for (int i = 0; i < MAX_DEVICES; i++) {
      devices[i].mac = "";
      devices[i].rssi = 0;
      devices[i].packetCount = 0;
      devices[i].isSuspicious = false;
      devices[i].deviceName = "";
      devices[i].serviceUUID = "";
      devices[i].beaconUUID = "";
      devices[i].lastSeen = 0;
      devices[i].display = true;
      devices[i].jammingAlerted = false;
      devices[i].isBLE = true;
      devices[i].macChangeCount = 0;
    }
    for (int i = 0; i < MAX_LINES; i++) {
      displayLines[i].text = "";
      displayLines[i].color = GREEN;
...
void blesnifferSetup() {
  LOGP("[BLE]", "sniffer setup");
  if (!radioStateTryAcquire(RADIO_BLE, 180)) return;
  RDLOG("[BLE] Sniffer setup");
  ::BleScan::pauseForExternalBleFeature();
  hardResetBleSnifferEngine();
  tft.fillScreen(TFT_BLACK);
  delay(40);
  BLEDevice::init("");
  delay(140);
...
void blesnifferLoop() {
  ...
  if(feature_active) sniffer.loop();
  }
...
void exit() {
  ...
  BLEDevice::deinit(true);
  delay(220);
  ::BleScan::resumeIdleBackgroundScanner();
}
}
